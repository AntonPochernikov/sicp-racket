  ...
  (cond [(primitive-procedure? procedure)
        ...
        [(compound-procedure? procedure)
         (eval-sequence
          ...
          (extend-environment
           ; get parameter names without lazy and lazy-memo postfixes
           (procedure-parameters-names procedure)
           (list-of-args arguments
                         ; need to determine wether we will
                         ; treat arg exp as a thunk or not
...
; Now we have 3 types of arguments
; lazy, lazy with memo and normal arguments.
; We will transform delayed arguments to appropriate thunks.
(define (list-of-args exps parameters env)
  (if (no-operands? exps)
      '()
      (let ([argument (first-operand exps)]
            [parameter (first-param parameters)])
        (cons (cond [(lazy-param? parameter)
                     (delay-it argument env)]
                    [(lazy-memo-param? parameter)
                     (delay-with-memo argument env)]
                    [else
...
(define (lazy-param-name arg) (car arg))
...
; Now we memoize only memoized-thunk
(define (force-it obj)
  (cond [(thunk? obj)
...
(define (delay-with-memo exp env)
...
; get parameters names to extend
(define (procedure-parameters-names p)
  (let ([params (procedure-parameters p)])
    (map (lambda (param)
           (if (or (lazy-param? param)
                   (lazy-memo-param? param))
               (lazy-param-name param)
               param))
         params)))
