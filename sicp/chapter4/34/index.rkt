...
(define (print-pair pair depth)
  (define (print-lazy value)   
    (cond [(thunk? value)
           (user-print (force-it value) (- depth 1))]
          [(evaluated-thunk? value)
           (user-print (thunk-value value) (- depth 1))]
          [else
           (user-print value (- depth 1))]))
  (cond [(eq? depth 0)
         (display "...)")]
        [(null? pair)
         (display '())]
        [else
         (let* ([env (procedure-environment pair)]
                [head (lookup-variable-value 'x env)]
                [tail (lookup-variable-value 'y env)])
           (display "(")
           (print-lazy head)
           (display " . ")
           (print-lazy tail)
...
(define (user-print object depth)
  (cond [(pair? object)
         (print-pair object depth)]
